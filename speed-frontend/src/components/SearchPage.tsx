...
const SearchPage: React.FC = () => {
    ...
    const fetchDefaultApprovedArticles = async () => {
      ...
      try {
          ...
          if (user?.role === 'Moderator') {
              ...
              const approvedResponse = await fetch(`${backendUrl}/api/articles/search?status=APPROVED`);
              const deniedResponse = await fetch(`${backendUrl}/api/articles/search?status=DENIED`);
    ...
    const handleSearch = async () => {
      ...
      try {
          let apiUrl = `${backendUrl}/api/articles/search?text=${encodeURIComponent(searchQuery)}&status=APPROVED`;
  
          if (user?.role === 'Moderator') {
              const approvedResponse = await fetch(`${backendUrl}/api/articles/search?text=${encodeURIComponent(searchQuery)}&status=APPROVED`);
              const deniedResponse = await fetch(`${backendUrl}/api/articles/search?text=${encodeURIComponent(searchQuery)}&status=DENIED`);
